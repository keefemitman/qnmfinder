...
def varpro(t, y, w, alpha, n, ada, bounds=None, max_nfev=None, **kwargs):
    ...
    if not (m == m2 or m == 2 * m2):
        raise Exception(
            "y and w must be the same or twice the length of t "
    ...
    def formJacobian(alpha, Phi, dPhi):
        ...
        # NOTE: empirically found to be better when 0
        ...
        Jac1 = U[:, myrank:m].dot(np.transpose(U[:, myrank:m]).dot(Jac1))
        T2 = np.diag(1 / s[0:myrank]).dot(V[0:myrank].dot(T2[0:n, :]))
